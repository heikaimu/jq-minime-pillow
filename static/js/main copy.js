...
<div class="cropper-box">
  ...
  <div class="full-screen-loading cropper-loading">
    ...
    <div class="loading-text-pending">AI Croping your photo</div>
  </div>
</div>
`
...
<footer class="cropper-footer">
    <p class="menu fx1 cropper plain">${myIcon('cropper')} ${menuCropper}</p>   
    <p class="menu fx1 save">${menuDirectUse}</p>
  </footer>
`
...
const templateDIYContainer = function (productPrice, productTitle) {
  ...
  <div class="combine-wrapper">
    ...
    <div class="combine-medium">
      <canvas id="minimeCanvas" />
    </div>
    <footer class="combine-footer">
      <ul class="avatar-combine-menus">
        <li class="item">
          <p class="button plain replace">${DIYReplace}</p>
        </li>
        <li class="item">
          <p class="button confirm">${DIYConfirm}</p>
        </li>
      </ul>
    </footer>
...
function getAI(blob) {
  ...
  return new Promise((resolve, reject) => {
    getPicSerectKeys().then(res => {
      ...
      // let { uid, signExpireTime, sign } = {
      //   sign: "27c61190c6b73cb2af1f5d7c5376565518dfe56834f80f40126cc28c8367e17a",
      //   signExpireTime: "1622182284",
      //   uid: "140982"
      // };
      // const postURL = 'http://www.picup.shop/api/v1/matting2?mattingType=3&crop=true';
      const postURL = `https://picupapi.tukeli.net/api/v1/matting2?mattingType=3&crop=true&uid=${uid}&signExpireTime=${signExpireTime}&sign=${sign}&faceAnalysis=true`;
      $.ajax({
        type: 'POST',
        url: postURL,
        data: data,
        ...
        headers: {
          ...
          // APIKEY: '3564d8b6c9b9403d80bbbd81909857e1'
        },
        success: (res) => {
          if (res.code === 0) {
...
function MinimePillow(option, callback) {
...
MinimePillow.prototype.addToCart = function () {
  ...
  $progressBar.find('.line').css('width', '0%');
  $progressBar.find('.percent').text('0%');

  const count = BlobList.length;
  let successCount = 0;

  BlobList.forEach(item => {
    const name = `${this.option.name}_${item.name}_${getRandomID()}`;
    uploadFileToS3(item.file, name).then(res => {
      successCount += 1;

      item.url = res;

      const percent = successCount / count;
      const rate = Math.floor(percent * 100) + '%'
      $progressBar.find('.line').css('width', rate);
      $progressBar.find('.percent').text(rate);
...
FileSelectState.prototype.render = function () {
  ...
  JQEle.find('.close-btn').click(function () {
    _this.minime.$containerEle.remove();
  })
...
CropperState.prototype.render = function () {
  ...
  _this.minime.chin = null;

  const JQEle = $(templateCropper);
  this.$loading = JQEle.find('.cropper-loading');
  this.$loading.hide();
  this.minime.$contentEle.html(JQEle);
...
CropperState.prototype.createPreview = function (JQEle) {
  ...
  // 添加预览图片
  const $image = $(`<img id="cropperImage" src="${getObjectUrl(this.minime.rawFile)}"/>`);
  JQEle.find('.cropper-content').html($image);
  ...
  // 打开裁剪
  $previewMenus.find('.cropper').click(function () {
    $previewMenus.remove();
  ...
  $previewMenus.find(".save").click(function () {
    _this.getAIFromBlob(_this.minime.rawFile);
  })
}
...
ListState.prototype.render = function () {
  ...
  _this.minime.skin = colorItems.eq(0).attr('_val');
  colorItems.each(function () {
    $(this).click(function () {
      $(this).addClass('active').siblings('.color-item').removeClass('active');
      _this.minime.skin = $(this).attr('_val');
      // 渲染列表
      _this.renderAICardPreview();
    })
  })
  ...
  // 点击锚地滚动到对应的组
  $achorList.find('.item').each(function () {
    $(this).click(function () {
      $(this).addClass('active').siblings('.item').removeClass('active');
      const name = $(this).attr('_val');
      JQEle.find('.card-list-wrapper').scrollToEle(JQEle.find(`.group-item[_val='${name}']`));
      _this.renderAICardPreview(name);
    })
  })

  // 卡片
  function renderCardList(productId) {
    const $cardList = $(_this.cardList());
    JQEle.find('.card-list-wrapper').html($cardList);

    JQEle.find('.card-list-wrapper').scrollEleIndex('group-item', function (index) {
      $achorList.find('.item').eq(index).addClass('active').siblings('.item').removeClass('active');
    })

    // 如果有ID
    if (productId) {
      const matchCard = JQEle.find(`.preview-card[_productId='${productId}']`);
      matchCard.addClass('active');
      if (matchCard.length > 0) {
        JQEle.find('.card-list-wrapper').scrollToEle(matchCard, 0, 10);
      }
    }
  }
  renderCardList(this.minime.option.productId);
...
ListState.prototype.achorList = function () {
  const liList = this.list.map(item => {
    return `<li class="item" _val="${item.name}"><p class="text">${item.name}</p></li>`
  })
...
/*
  卡片列表
*/
ListState.prototype.cardList = function () {
  let listStr = '';
  listStr += '<ul class="group-list">';
  for (let i = 0; i < this.list.length; i++) {
    const group = this.list[i];
    listStr += `<li class="group-item" _val="${group.name}">`;
    listStr += `<h2 class="group-name">${group.name}</h2>`;
    listStr += `<ul class="card-list">`;
    for (let c = 0; c < group.images.length; c++) {
      const option = group.images[c];
      listStr += `<li class="card-item">`;
      listStr += `<div class="preview-card" id="${option.eleID}" _productId="${option.id}">`;
      listStr += `<img src="" alt="">`;
      // listStr += `<p class="text">${option.name}</p>`;
...
// 渲染卡片预览列表图片
ListState.prototype.renderAICardPreview = function (currentGroupName) {
  // 首先全部加上loading
  ...
  const list = this.getRenderOrderList(currentGroupName);
  for (let i = 0; i < list.length; i++) {
    const group = list[i];
    for (let c = 0; c < group.images.length; c++) {
      const option = group.images[c];
      this.queue.push(this.requestInstance.request(option));
    }
  }
  Promise.all(this.queue);
}

// 将当前组的数据作为渲染的第一组，这样能更快的渲染出当屏内容 TODO
ListState.prototype.getRenderOrderList = function(name) {
  const currentGroupName = name ? name : $(".anchor-list").find(".item.active").attr("_val");
  if (!currentGroupName) {
    return this.list;
  } else {
    const index = this.list.findIndex(item => item.name === currentGroupName);
    const newList = this.list.slice(0);
    const currentGroupList = newList.splice(index, 1);
    newList.unshift(...currentGroupList);
    return newList;
  }
}
