...
const AIPending = 'AI Croping your photo';
...
CropperState.prototype.render = function () {
  ...
  // 首先将源文件转成base64并且赋值
  this.currentPicture = getObjectUrl(this.minime.rawFile);
...
CropperState.prototype.createPreview = function (JQEle) {
  ...
  $previewContent.find('.rotate').on('click', function(e) {
    ...
    blobToDataURL(_this.minime.rawFile).then(url => {
      rotateImage(url, _this.angle).then(src => {
...
CropperState.prototype.getAIFromBlob = function (blob) {
  ...
  _this.$loading.find('.loading-text-pending').text(AIPending);
  getAI(blob).then(avatarData => {
    _this.$loading.find('.loading-pending').fadeOut().siblings('.loading-success').fadeIn();
    _this.$loading.find('.loading-text-pending').text(AISuccess);
    setTimeout(() => {
    ...
    _this.$loading.find('.loading-text-pending').text(msg);
  })
}
